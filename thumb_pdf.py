...
def thumb_pdf(pdf_dir, thumbs_dir):
  ...
  for i,p in enumerate(pdf_files):
    ...
    thumb_path = os.path.join(thumbs_dir, p + '.jpg')
    ...
    pp = Popen(['convert', '%s[0-1]' % (pdf_path, ), '-thumbnail', 'x800', os.path.join(Config.tmp_dir, 'thumb.png')])
    t0 = time.time()
